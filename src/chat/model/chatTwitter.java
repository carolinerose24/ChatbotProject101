...
public class ChatTwitter
{
	...
	private String[] createIgnoredWordArray()
	{
		...
		Scanner wordScanner = new Scanner(this.getClass().getResourceAsStream("data/commonWords.txt")); // new Scanner(fileText);

		while (wordScanner.hasNextLine()) // read it w/o saving (actually destroys the line) ->all this to get the size
		{
		...
		wordScanner.close(); // then CLOSE THE FILE

		// Alternative file loading method.
		// Uses the InputStream class
		// Notice the lack of try/catch

		wordScanner = new Scanner(this.getClass().getResourceAsStream("data/commonWords.txt")); // grab as stream ->new way to read info
		for (int index = 0; index < boringWords.length; index++) // file stream -> java structure/data type that allows info to be sent as binary
																	// data w/a defined endpoint
		{
			boringWords[index] = wordScanner.nextLine(); // output and input stream --> system.out or .in
		}
